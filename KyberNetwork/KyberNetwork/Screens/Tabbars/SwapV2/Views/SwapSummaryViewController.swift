...
class SwapSummaryViewController: KNBaseViewController {
  ...
  func openTransactionSettings(gasLimit: BigInt, settings: SwapTransactionSettings) {
    ...
    let vm = TransactionSettingsViewModel(gasLimit: gasLimit, selectType: selectedGasPriceType, rate: viewModel.swapObject.rate, defaultOpenAdvancedMode: settings.advanced != nil)
