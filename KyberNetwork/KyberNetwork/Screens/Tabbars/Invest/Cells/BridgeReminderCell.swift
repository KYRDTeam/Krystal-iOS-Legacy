...
class BridgeReminderCell: UITableViewCell {
  ...
  func updateReminderText(crossChainFee: String, miniAmount: String, maxAmount: String, minFeeString: String, maxFeeString: String) {
    let reminder1 = minFeeString == maxFeeString ? "•  Transaction fee is \(crossChainFee)%, gas fee \(minFeeString) & it is paid to nodes facilitating token transfer" : "•  Transaction fee is \(crossChainFee)%, gas fee \(minFeeString) (max \(maxFeeString)) & it is paid to nodes facilitating token transfer \n"
