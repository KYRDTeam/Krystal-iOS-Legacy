...
extension KAddress {
  func getDistintID() -> String {
    if let wallet = WalletManager.shared.getWallet(forAddress: self), wallet.importType == .mnemonic {
  ...
  func getSuperProperty() -> [String: String] {
    ...
    if let wallet = WalletManager.shared.getWallet(forAddress: self), wallet.importType == .mnemonic {
      if let evmAddress = WalletManager.shared.getAllAddresses2(walletID: wallet.id, addressType: .evm).first {
        result["ethereum_address"] = evmAddress.addressString.lowercased()
      }
      
      if let solAddress = WalletManager.shared.getAllAddresses2(walletID: wallet.id, addressType: .solana).first {
