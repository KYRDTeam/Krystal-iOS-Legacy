...
struct HomesteadSigner: Signer {
    ...
    func values(transaction: SignTransaction, signature: Data) -> (r: BigInt, s: BigInt, v: BigInt) {
        ...
        let r = BigInt(sign: .plus, magnitude: BigUInt(try! Data(signature[..<32])))
        let s = BigInt(sign: .plus, magnitude: BigUInt(try! Data(signature[32..<64])))
        let v = BigInt(sign: .plus, magnitude: BigUInt(try! Data(bytes: [signature[64] + 27])))
        return (r, s, v)
    }
}
