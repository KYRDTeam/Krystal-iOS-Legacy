...
//    self.delegate?.dappBrowserHomeViewController(self, run: .showAllRecently)
  ...
  @IBAction func searchTextButtonTapped(_ sender: UIButton) {
    guard self.searchTextField.isFirstResponder else {
        self.openBrowserScreen(searchText: self.searchTextField.text ?? "")
      return
    }
  ...
  func openBrowserScreen(searchText: String) {
      if AppState.shared.currentAddress.isWatchWallet { return }
  ...
  func tagPressed(_ title: String, tagView: TagView, sender: TagListView) {
    ...
    case 1:
      ...
      if let unwrap = filtered?.url {
          self.openBrowserScreen(searchText: unwrap)
      }
    case 2:
