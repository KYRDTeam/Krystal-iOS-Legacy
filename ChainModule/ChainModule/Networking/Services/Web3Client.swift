...
class Web3Client: NSObject {
    ...
    func request<T: Web3Request>(request: T, completion: @escaping (Swift.Result<T.Response, AnyError>) -> Void) {
        ...
        case .function(let command):
            webView.evaluate(expression: JSFunction<T.Response>(command), completionHandler: { result in
                switch result {
...
extension Web3Client: WKNavigationDelegate {
