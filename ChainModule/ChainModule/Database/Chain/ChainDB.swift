...
public class ChainDB {
    ...
    let chainObserver = ChainObserver()
    ...
    func getSmartContracts(chainID: Int, type: String) -> [ChainSmartContractObject] {
        let realm = try! Realm()
        return realm.objects(ChainSmartContractObject.self).where { object in
            return object.chainID == chainID && object.type == type
        }.toArray()
    }
    ...
    func getUrls(chainID: Int, type: String) -> [ChainUrlObject] {
        let realm = try! Realm()
        return realm.objects(ChainUrlObject.self).where { object in
            return object.chainID == chainID && object.type == type
        }.toArray()
    ...
    public func getConfig(chainID: Int, key: String) -> String? {
        let realm = try! Realm()
        return realm.objects(ChainConfigObject.self).where { object in
            return object.chainID == chainID && object.name == key
        }.first?.value
    }
